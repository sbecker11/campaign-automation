...
def test_generate_reports_handles_internal_error(tmp_path, caplog, monkeypatch):
    ...
    assert any("Failed to generate campaign_instance.json" in r.message for r in caplog.records)
...
def test_instance_generator_main_smoke():
    # Run the module as a script to cover the __main__ block
    # Execute from current project root (pytest sets cwd to repo root)
    result = subprocess.run([sys.executable, "-m", "src.instance_generator"], capture_output=True, text=True)
    # It should exit successfully and print success message
    assert result.returncode == 0
    assert "Test campaign_instance.json generated" in (result.stdout + result.stderr)

def test_instance_generator_main_inprocess(tmp_path, monkeypatch):
    ...
    runpy.run_module("src.instance_generator", run_name="__main__")
    # Verify campaign_instance.json was written under ./temp/
    assert (temp_dir / "campaign_instance.json").exists()
