...
def test_generate_reports_handles_internal_error(tmp_path, caplog, monkeypatch):
    ...
    monkeypatch.setattr(gen, "_create_generation_report", lambda *a, **k: (_ for _ in ()).throw(RuntimeError("boom")))
    # Should not raise; error is logged
    gen.generate_reports(results=[], brief={}, output_dir=tmp_path)
    assert any("Failed to generate reports" in r.message for r in caplog.records)
...
def test_report_generator_main_smoke():
    ...
    assert "Test reports generated" in (result.stdout + result.stderr)
...
def test_report_generator_main_inprocess(tmp_path, monkeypatch):
    ...
    # Verify reports were written under ./temp/
    assert (temp_dir / "generation_report.json").exists()
    assert (temp_dir / "compliance_report.json").exists()
